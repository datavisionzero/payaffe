...
- Observability should use external self-hosted services according to [observability-baseline.md](observability-baseline.md): OpenTelemetry/OTLP as the telemetry standard, optional Serilog behind `ILogger<T>` for .NET hosts, Grafana Alloy as the preferred collector, Grafana LGTM Stack for logs, dashboards, traces, and metrics, plus GlitchTip for error reporting. Production setups document telemetry retention, sampling, minimum dashboards, and minimum alerts as described there.
